...
- Embeddable audio lives in `content/audio/` and is auto-discovered by `src/lib/audio.ts` the same way — drop an audio file in and reference it from a post body (see audio embeds below).
...
- Front matter is parsed with `front-matter`. The body is split into an ordered `blocks` array (`PostBlock[]`): lines matching `:::pdf <name>` become `{ type: "pdf" }` embeds and `:::audio <name>` become `{ type: "audio" }` embeds; everything between them is rendered to HTML with `remark` + `remark-gfm` + `remark-html` using `sanitize: false`. `Post.tsx` walks `blocks`, injecting prose via `dangerouslySetInnerHTML` and rendering `<PdfDoc>` / `<AudioDoc>` for embed blocks, so prose and embeds interleave in author order. Post HTML is trusted — this is a single-author blog. (`post.html` is still exposed as the concatenated prose, but `blocks` is what's rendered.)
...
- **Audio embeds:** `<AudioDoc>` (`src/components/AudioDoc.tsx`) renders an audio file as an inline custom player (play/pause button, scrubbable progress bar, elapsed/total time, open-original link) built on the native `<audio>` element. Files are resolved via `src/lib/audio.ts`. Unlike `PdfDoc`, it has no heavy dependency, so `Post.tsx` imports it directly (no `React.lazy`/`Suspense`). `vite.config.ts` `assetsInlineLimit` also forces audio files to never inline, so they emit as real fetchable files the element can stream.
- **Date-based publishing:** only posts with `date <= today` (local date) appear. A future-dated post simply won't show until that day.
- **Day numbering:** every published post gets `dayNumber` in chronological order, pinned included (so the pinned "Start Here" piece is Day 1). `pinned: true` only anchors a post at the top of the feed — it no longer changes the label. Prev/next adjacency (`getAdjacentPosts`) runs over *all* published posts chronologically, including pinned.
- Slug comes from the filename: `YYYY-MM-DD-the-slug.md` -> `the-slug`. Routes are flat: `/` (Home) and `/:slug` (Post).
- **Reading time:** computed from the body word count at `READING_WPM` (112.5 wpm — half a typical ~225 wpm pace, since the raw estimate reads too fast for this writing). An optional `reading_time` front-matter field (minutes) overrides it via `resolveReadingMinutes` — necessary for PDF-heavy posts, since the embed's text isn't in the Markdown body. Missing/blank/non-positive values fall back to the computed estimate.
...
**Audio embeds:** drop an audio file (`.mp3`, `.m4a`, `.wav`, `.ogg`, `.aac`) in `content/audio/` and embed it with `:::audio <filename>` on its own line (extension optional). It renders as an inline player (play/pause, scrubber, time, open-original link). A name with no matching file shows an "Audio not found" notice. As with PDFs, the player's runtime isn't counted in reading time — set `reading_time` for an audio-heavy piece. See `content/writing/_formatting-guide.md` for the author-facing reference.
